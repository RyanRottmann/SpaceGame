...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func fireTorepedo2(){
        ...
        var actionArray = [SKAction]()
        actionArray.append(SKAction.move(to: CGPoint(x: player.position.x + 30, y: UIScreen.main.bounds.height), duration: animationDuration))
        actionArray.append(SKAction.removeFromParent())
    ...
    func fireTorepedo4(touchLocation: CGPoint){
        self.run(SKAction.playSoundFileNamed("torpedo.mp3", waitForCompletion: false))
        let torpedoNode = SKSpriteNode(imageNamed: "torpedo")
        ...
        var actionArray = [SKAction]()
        actionArray.append(SKAction.move(to: touchLocation, duration: animationDuration))
    ...
    func fireTorepedo5(touchLocation: CGPoint){
        self.run(SKAction.playSoundFileNamed("torpedo.mp3", waitForCompletion: false))
        let torpedoNode = SKSpriteNode(imageNamed: "torpedo")
        let reversePoint = CGPoint(x: -touchLocation.x, y: touchLocation.y)
        ...
        var actionArray = [SKAction]()
        actionArray.append(SKAction.move(to: reversePoint, duration: animationDuration))
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if(level > 3){
            ...
            fireTorepedo3()
            if(level > 5){
                fireTorepedo4(touchLocation: location)
                if(level > 10){
                    fireTorepedo5(touchLocation: location)
                }
            }
        }
